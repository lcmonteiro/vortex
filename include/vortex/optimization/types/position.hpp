...
#include "vortex/optimization/graph_edge.hpp"
#include "vortex/optimization/graph_node.hpp"

namespace vortex::optimization::types {

/// @brief A scalar-generic 2D position / translation.
/// Used both as a node estimation type and as an edge measurement type.
/// Being templated on the scalar type lets the same value participate:
/// - in real-valued residual evaluation (`Number == double`)
/// - in dual-number Jacobian evaluation (forward-mode automatic differentiation).
...
/// Participates in:
/// - `position_location_edge` (absolute position prior) and
...
struct position_node : position_node_base<position_node<Number>, Number> {
  using base_type = position_node_base<position_node<Number>, Number>;
  using base_type::base_type;

  /// @brief Type of the delta vector used for updating the node's estimation.
  /// @tparam T The scalar type of the delta vector.
  template <class T>
  using update_vector = typename base_type::template update_vector<T>;

  /// @brief Manifold retraction: adds a tangent-space increment to the current estimate.
  /// Templated on the increment scalar type so it works both for:
  /// - in real-valued optimizer steps and
  /// - in dual-number seeding (exact Jacobians via forward-mode automatic differentiation).
  /// @param delta Increment vector of length 2: [dx, dy].
  /// @return A new `position` with the updated coordinates.
  template <class T>
  auto plus(const update_vector<T>& delta) const {
    return position<T>{this->estimation().x + delta[0], this->estimation().y + delta[1]};
  }
};
...
template <class Number>
struct position_distance_edge
    : position_distance_edge_base<position_distance_edge<Number>, Number> {
  using base_type = position_distance_edge_base<position_distance_edge<Number>, Number>;
  using base_type::base_type;

  /// @brief Alias for the error vector type, templated on the scalar type.
  /// @tparam T Scalar type (`Number` for residual, or a dual number for Jacobians).
  template <class T>
  using error_vector = typename base_type::template error_vector<T>;

  /// @brief Computes the residual between the measured and observed relative translation.
  /// @tparam T
...
template <class Number>
struct position_location_edge
    : position_location_edge_base<position_location_edge<Number>, Number> {
  using base_type = position_location_edge_base<position_location_edge<Number>, Number>;
  using base_type::base_type;
  ...
  /// @brief Computes the residual between the estimated and measured absolute position.
  /// @tparam T Scalar type (`Number` for residual, or a dual number for Jacobians).
  /// @param a Estimate of the position.
  /// @return Error vector [a.x − measured_x, a.y − measured_y].
  template <class T>
  auto error(const position<T>& a) -> error_vector<T> {
