/// ===============================================================================================
/// @file
/// @brief Global `operator new`/`operator delete` overrides backing
/// `bounded_arena_resource::guard`.
/// ===============================================================================================
...
auto check() -> void {
  if (g_alloc_forbidden) {
    ...
    throw vortex::test::memory_violation{
        "heap allocation inside a bounded_arena_resource::guard scope"};
  }
}
...
bounded_arena_resource::guard::guard() noexcept { g_alloc_forbidden = true; }
bounded_arena_resource::guard::~guard() noexcept { g_alloc_forbidden = false; }
}  // namespace vortex::test
...
auto operator delete[](void* p) noexcept -> void { release(p); }
auto operator delete(void* p, std::size_t) noexcept -> void { release(p); }
auto operator delete[](void* p, std::size_t) noexcept -> void { release(p); }
auto operator delete(void* p, std::align_val_t) noexcept -> void { release(p); }
auto operator delete[](void* p, std::align_val_t) noexcept -> void { release(p); }
auto operator delete(void* p, std::size_t, std::align_val_t) noexcept -> void { release(p); }
auto operator delete[](void* p, std::size_t, std::align_val_t) noexcept -> void { release(p); }
auto operator delete(void* p, const std::nothrow_t&) noexcept -> void { release(p); }
auto operator delete[](void* p, const std::nothrow_t&) noexcept -> void { release(p); }
auto operator delete(void* p, std::align_val_t, const std::nothrow_t&) noexcept -> void {
  release(p);
}
auto operator delete[](void* p, std::align_val_t, const std::nothrow_t&) noexcept -> void {
  release(p);
